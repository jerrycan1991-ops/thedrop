...
The schema migration is committed, so a fresh clone only needs `db:upgrade`. When you
change a model and need a new revision, the order matters:

```bash
pnpm db:upgrade && pnpm db:revision "what changed" && pnpm db:upgrade
```

`db:revision` autogenerates by diffing the models against the **live** database, so the
database must already be at head before you generate. Running `db:revision` first fails
with `Target database is not up to date` — that error means "upgrade before you
generate", not "something is broken".

Review every generated migration before applying it. Two things autogenerate gets wrong
in this schema:

- **Circular foreign keys.** `articles.hero_media_id` ↔ `media_assets.article_id` have
  no valid CREATE TABLE ordering. The model marks one side `use_alter=True` so Alembic
  can sort the tables, but `op.create_table` then silently omits that constraint — it
  must be added explicitly with `op.create_foreign_key` at the end of `upgrade()`, and
  dropped first in `downgrade()`. See the manual block in the initial schema migration.
- **Server defaults and enum changes** are frequently mis-detected. Check the diff.
